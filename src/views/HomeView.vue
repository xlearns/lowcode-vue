...
import { useWidge } from "@/hooks/useWidge";
...
let { CONFIG, status, drawerMainButton, options, activeNames, widgetList } =
  useWidge();
let { bgImg } = useStyle();
...
function onDrop(event, i) {
  ...
  let obj = {
    ...
    styles: cloneDeep(currentWidget.value.styles),
  };
...
const whileList = ["canvasBg", "boxBg"];
function handleKeepActive(e) {
  const target = e.target || e.srcElement;
  if (whileList.includes(target.className)) {
    isMenuDown.value = false;
  } else {
    isMenuDown.value = true;
  }
  current.value && onBlurs(current.value);
}
